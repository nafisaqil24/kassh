...
const SECTION_TITLE =
  'font-size:14px;font-weight:700;color:#1E293B;margin:0 0 8px 0;letter-spacing:0.03em;';

const TABLE_STYLE =
  'width:100%;border-collapse:collapse;font-size:11px;color:#1E293B;';

const TH_STYLE =
  'border:1px solid #E2E8F0;background:#EFF6FF;padding:6px 8px;text-align:left;font-weight:700;';

const TD_STYLE = 'border:1px solid #E2E8F0;padding:5px 8px;vertical-align:top;';
...
function buildHeaderHtml(data: LaporanPayload, tanggalCetak: string): string {
  return `<header style="text-align:center;margin-bottom:20px;border-bottom:3px solid #4F46E5;padding-bottom:14px;">
      <div style="font-size:11px;letter-spacing:0.25em;text-transform:uppercase;color:#64748B;">Laporan Kas</div>
      <h1 style="font-size:26px;font-weight:800;color:#1E293B;margin:6px 0 4px;letter-spacing:0.08em;">UANG KAS</h1>
      <div style="display:inline-block;background:#E0E7FF;border:1px solid #C7D2FE;padding:4px 14px;border-radius:999px;font-size:12px;font-weight:700;color:#4F46E5;">${escapeHtml(data.periode)}</div>
      <div style="font-size:11px;color:#64748B;margin-top:8px;">Nominal kas per pertemuan: <strong>${rupiah(data.nominalPerSesi)}</strong> · Dicetak: ${escapeHtml(tanggalCetak)}</div>
    </header>`;
}

function buildFooterHtml(tanggalCetak: string): string {
  return `<footer style="margin-top:24px;border-top:1px solid #E2E8F0;padding-top:8px;font-size:9px;color:#64748B;text-align:center;">
...
function sectionHtml(title: string, bodyHtml: string, pageBreakBefore: boolean = false): string {
  ...
  return `<section style="margin:0 0 18px 0;${breakStyle}">
      <div style="border-bottom:2px solid #4F46E5;margin-bottom:8px;padding-bottom:4px;">
...
function buildGridHtml(data: LaporanPayload): string {
  if (data.kolomPertemuan.length === 0) {
    return '<p style="color:#64748B;font-size:11px;">Belum ada data pertemuan.</p>';
  }
  ...
  const rows = data.grid
    .map((row) => {
      const cells = row.statuses
        .map(
          (st) =>
            `<td style="${TD_STYLE};text-align:center;color:${st ? '#047857' : '#64748B'};font-weight:700;">${st ? '✓' : '-'}</td>`
        )
        .join('');
...
function buildRekapHtml(data: LaporanPayload): string {
  const card = (label: string, value: string, color: string, note: string) =>
    `<div style="flex:1;min-width:150px;border:1px solid #E2E8F0;background:#F8FAFC;padding:10px 12px;border-radius:4px;">
      <div style="font-size:10px;color:#64748B;text-transform:uppercase;letter-spacing:0.05em;">${escapeHtml(label)}</div>
      <div style="font-size:16px;font-weight:700;color:${color};margin-top:4px;">${escapeHtml(value)}</div>
      <div style="font-size:9px;color:#64748B;margin-top:4px;">${escapeHtml(note)}</div>
    </div>`;

  const cards = `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;">
    ${card('Total Pemasukan', rupiah(data.rekap.totalPemasukan), '#047857', `Dari ${data.rekap.totalLunasCount} pembayaran lunas`)}
    ${card('Total Pengeluaran', rupiah(data.rekap.totalPengeluaran), '#B45309', `Dari ${data.rekap.jumlahPengeluaran} catatan`)}
    ${card('Saldo Kas Bersih', rupiah(data.rekap.saldoKas), data.rekap.saldoKas < 0 ? '#DC2626' : '#047857', 'Pemasukan − Pengeluaran')}
  </div>`;

  let tunggakanHtml: string;
  if (data.tunggakan.length === 0) {
    tunggakanHtml =
      '<p style="font-size:11px;color:#047857;background:#ECFDF5;border:1px solid #A7F3D0;padding:8px 10px;border-radius:4px;margin:0;">Semua anggota sudah melunasi seluruh kas pertemuan.</p>';
  } else {
    const rows = data.tunggakan
      .map(
        (t) =>
          `<tr style="${TR_STYLE}"><td style="${TD_STYLE};">${escapeHtml(t.nama)}</td><td style="${TD_STYLE};text-align:center;">${t.jumlahBelumBayar} pertemuan</td><td style="${TD_STYLE};text-align:right;font-weight:700;color:#DC2626;">${rupiah(t.totalTunggakanRupiah)}</td></tr>`
      )
      .join('');
  ...
  return `${cards}<div style="font-size:12px;font-weight:700;color:#1E293B;margin:8px 0 6px;">Daftar Tunggakan</div>${tunggakanHtml}`;
}

function buildPemasukanHtml(data: LaporanPayload): string {
  if (data.riwayatPemasukan.length === 0) {
    return '<p style="color:#64748B;font-size:11px;">Belum ada pemasukan tercatat.</p>';
  }
  const rows = data.riwayatPemasukan
    .map(
      (p) =>
        `<tr style="${TR_STYLE}"><td style="${TD_STYLE};font-weight:600;">${escapeHtml(p.nama)}</td><td style="${TD_STYLE};">${escapeHtml(p.hari)}, ${escapeHtml(p.tanggal)} ${escapeHtml(data.periode)}</td><td style="${TD_STYLE};text-align:right;font-weight:700;color:#047857;">${rupiah(p.nominal)}</td></tr>`
    )
    .join('');
  return `<table style="${TABLE_STYLE}">
...
function buildPengeluaranHtml(data: LaporanPayload): string {
  if (data.pengeluaran.length === 0) {
    return '<p style="color:#64748B;font-size:11px;">Belum ada data pengeluaran kas.</p>';
  }
  const rows = data.pengeluaran
    .map(
      (p) =>
        `<tr style="${TR_STYLE}"><td style="${TD_STYLE};">${escapeHtml(p.tanggal)}</td><td style="${TD_STYLE};">${escapeHtml(p.keterangan)}</td><td style="${TD_STYLE};text-align:right;font-weight:700;color:#B45309;">${rupiah(p.nominal)}</td></tr>`
    )
    .join('');
  return `<table style="${TABLE_STYLE}">
...
async function renderToPdfBuffer(
  ...
  wrapper.style.cssText =
    `position:fixed;left:-10000px;top:0;width:${targetWidth}px;background:#FFFFFF;z-index:-1;`;
